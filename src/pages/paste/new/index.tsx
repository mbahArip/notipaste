...
export default function NewPastePage() {
  ...
  const [pastePrivacy, setPastePrivacy] = useState<Key[]>([...new Set(['public'])]);
  const [pasteExpiration, setPasteExpiration] = useState<Key[]>([...new Set(['0'])]);
  const [pastePassword, setPastePassword] = useState<string>('');
  const [pasteIdentifier, setPasteIdentifier] = useState<string>('');
  const [pasteDeleteToken] = useState<string>(generateId());
  const [pasteDescription, setPasteDescription] = useState<string>('');
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setPublishedId(pasteIdentifier ? pasteIdentifier : pasteId);
    } catch (error: any) {
            ...
            {user ? (
              ...
              <div className='flex flex-col gap-1 rounded-medium border border-divider bg-danger/10 p-2'>
                ...
                <Input
                  variant='flat'
                  color='danger'
                  value={`${process.env.NEXT_PUBLIC_VERCEL_URL}/delete?token=${pasteDeleteToken}`}
                  isReadOnly
                  ...
                  endContent={
                    <Button
                      ...
                      onPress={() => {
                        try {
                          const url = new URL(`/delete?token=${pasteDeleteToken}`, process.env.NEXT_PUBLIC_VERCEL_URL);
                          copyText(url.toString());
                        } catch (error: any) {
                          console.error(error);
                          toast.error(error.message);
